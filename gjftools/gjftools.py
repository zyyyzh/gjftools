...
class gjfdata():
    ...
    def get_model_file(self,
                       modelfile: str,
                       force_c_m: bool=False):
        '''
        read head and tail from model gjf file.
        Args:
            force_c_m: force to use c_m para in model file
        '''
    ...
    def write_xyz(self,
        ...
        if to_str_list:  # output info as list of string directly
            return out_xyz_list
    ...
    def _compair_basis_element(self):
        """Compare and modify basis info according to element in structure.
        """
        original_tail = deepcopy(self.tail)
        basis_list = []  # [basis_name, [element list], [line_idx list]]

        # read basis info from tail
        for j, line in enumerate(original_tail):
            if line.strip() == '****':
                if original_tail[j-2].split()[-1] == '0':
                    basis_name = original_tail[j-1].strip()
                    element_list = original_tail[j-2].split()[:-1]
                    # detect pesudo basis set definition
                    if original_tail[j+2].strip() == original_tail[j-2].strip():
                        line_idx_list = [j-2, j+2]
                    else:  # no pesudo basis
                        line_idx_list = [j-2]
                    basis_list.append([basis_name, element_list, line_idx_list])

                else:  # defined by atom index
                    print('Warning: basis not defined by element, abort basis check!')
                    return None
        ...
        if len(basis_list) > 0:
            ...
            atom_element_set = set(self.atom_element)
            # all elements in basis
            basis_element_set = set()
            for basis in basis_list:
                basis_element_set.update(basis[1])  # element_list
            
            # more elements in structure than in basis
            append_element_set = atom_element_set.difference(basis_element_set)
            # more elements in basis than in structure
            remove_element_set = basis_element_set.difference(atom_element_set)

            if append_element_set:
                # add all new elements to the first basis element list
                basis_list[0][1].extend(list(append_element_set))
            
            if remove_element_set:
                # remove one by one
                for e in remove_element_set:
                    # find where it was and delete
                    for basis in basis_list:
                        if e in basis[1]:
                            basis[1].remove(e)
        
        # format new tail
        basis_start_idx = basis_list[0][2][0]
        basis_end_idx = basis_list[-1][2][-1] + 2

        new_basis_element_lines = []
        for basis in basis_list:
            if len(basis[1]) == 0:  # no element left
                continue
            if len(basis[2]) == 1:  # non-pesudo basis
                basis_lines = [
                    f"{' '.join(basis[1])} 0\n",
                    f"{basis[0]}\n",
                    "****\n",
                ]
            elif len(basis[2]) == 2:  # pesudo basis
                basis_lines = [
                    f"{' '.join(basis[1])} 0\n",
                    f"{basis[0]}\n",
                    "****\n",
            ...
            new_basis_element_lines.extend(basis_lines)

        self.tail = original_tail[:basis_start_idx] +\
            new_basis_element_lines + original_tail[basis_end_idx:]
    ...
    def remove_atom(self,
                    atom_idx: Union[List[int],int]=1,
                    ):
        '''
        Remove one atom from the body.
        atom_idx: start from 1
        '''
        if isinstance(atom_idx, int):
            atom_idx = [atom_idx]

        atom_idx.sort(reverse=True)
        for index in atom_idx:
            if index <= 0:
                raise RuntimeError('Only positive integers are accepted!')
            self.atom_element.pop(index-1)
            self.atom_type.pop(index-1)
            self.atom_info.pop(index-1)
            self.atom_coord.pop(index-1)
            self.atom_num -= 1
